...
function DisplayScreen({ session }) {
  ...
  useEffect(() => {
    load()
    const channel = supabase.channel(`display-${session.id}`, { config: { broadcast: { self: true } } })
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'vote_agenda', filter: `session_id=eq.${session.id}` }, load)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'vote_participants', filter: `session_id=eq.${session.id}` }, load)
...
function ModeratorView() {
  ...
  const broadcastTimer = useCallback(async (seconds, running) => {
    ...
    await supabase.channel(`display-${activeSession.id}`).send({
