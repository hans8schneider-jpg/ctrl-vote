...
const css = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .reaction-btn { padding: 10px 16px; border: 1px solid ${G.border}; background: transparent; cursor: pointer; font-size: 11px; font-weight: 700; letter-spacing: 1px; transition: all 0.2s; border-radius: 4px; color: ${G.text2}; }
...
function ParticipantView({ session, participant }) {
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel(`participant-${session.id}-${participant.id}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'vote_polls', filter: `session_id=eq.${session.id}` }, load)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'vote_responses' }, load)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'vote_speakers', filter: `session_id=eq.${session.id}` }, load)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'vote_sessions', filter: `id=eq.${session.id}` }, load)
      .subscribe((status) => { console.log('Participant realtime:', status) })
    // Fallback poll every 5s for iPhone
        ...
        <div className="pview-content">
          ...
          <div className="speaker-card">
            ...
            <div className="reaction-bar">
              {[['SOUHLAS','souhlas'],['NAMITKA','namitka'],['OTAZKA','otazka']].map(([icon,type]) => (
...
function DisplayScreen({ session }) {
  ...
  // Timer sync via broadcast
  useEffect(() => {
    const ch = supabase.channel(`timer-${session.id}`)
                ...
                <div className="reactions-display">
                  {reactions.map(r => (
                    <div key={r.id} className="reaction-pill">
                      <span style={{ fontSize:18 }}>{r.reaction==='souhlas'?'SOUHLAS':r.reaction==='namitka'?'NAMITKA':'OTAZKA'}</span>
              ...
              <div className="mod-panel">
                ...
                {reactions.map(r => (
                  <div key={r.id} className="reaction-mod-item">
                    ...
                    <span style={{ fontSize:11, color:G.text2, marginLeft:'auto' }}>{r.reaction==='souhlas'?'SOUHLAS':r.reaction==='namitka'?'NAMITKA':'OTAZKA'}</span>
                  </div>
                ))}
              </div>
            )}
